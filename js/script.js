...
$(document).ready((function(){
    $(".burger_box , .overlay").click((function(s){
      $(".nav_burger, .overlay,.burger_logo,.burger_menu_box").toggleClass("active");
...
const catalogNames = document.querySelectorAll('.nav_link_ar');

catalogNames.forEach(function(item){
  item.addEventListener("mouseenter", function(item){
    this.classList.add("active")
    const dopInf = item.target.closest('.product_item').querySelector('.product_item_info').classList.add('active')
  })
...
const catalogBoxes = document.querySelectorAll('.nav_sublists_box');

catalogBoxes.forEach(function(item){
  item.addEventListener("mouseenter", function(item){
    this.classList.add("active")
  })
  item.addEventListener("mouseleave", function(item){
...
/* categories_filt end */
/* main_menu start */
const zItems = document.querySelectorAll('.product_item');
...
document.addEventListener('mouseover', function (event) {
    if (event.target.hasAttribute('data-action')) {
        ...
        const dopInfoIn = productItem.querySelector('.product_item_info')
        dopInfoIn.classList.add('active')
    }
})
...
document.addEventListener('mouseout', function (event) {
    if (event.target.hasAttribute('data-action')) {
        const productItem = event.target.closest('.product_item')
        const dopInfoIn = productItem.querySelector('.product_item_info')
        dopInfoIn.classList.remove('active')
    }
})
...
/*map_tabs start */
  ...
  tabHeaders.forEach(tasHandler);
  function   tasHandler(item){
    item.addEventListener("click", tabsClick);
  }
  function tabsClick (){
    const tabId = this.dataset.tab;
    document.querySelectorAll('[data-tab-content]').forEach(function (item) {
      item.classList.add('tab_content_hide');
    });
    document.getElementById(tabId).classList.toggle("tab_content_hide");
  }
/*map_tabs end */
/*Lazy_load_map start */
